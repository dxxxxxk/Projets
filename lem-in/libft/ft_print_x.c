...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/10/15 16:41:23 by rchaigno          #+#    #+#             */
/*   Updated: 2019/10/15 16:41:24 by rchaigno         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int				ft_print_x(t_opt *val, va_list args, int ret)
{
	...
	str = ft_itoa_num(stock, 16, 0);
	i = 0;
	len = ft_strlen(str);
	ret = ft_perf(val, ret, str, len);
	if (val->preci >= 0)
		ret = ft_op_x_1(val, ret, str, len);
	else
		ret = ft_op_x_2(val, ret, str);
	len = ft_strlen(str);
	if (str[i] != '-' && val->p == '+')
		len = len + 1;
	if (val->m == '-' && val->larg > 0)
